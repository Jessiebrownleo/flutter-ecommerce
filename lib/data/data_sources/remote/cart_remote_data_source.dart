...
class CartRemoteDataSourceSourceImpl implements CartRemoteDataSource {
  ...
  Future<CartItemModel> addToCart(CartItemModel cartItem, String token) async {
    print('🛒 [DEBUG] Adding cart item to API...');
    print('🛒 [DEBUG] Product ID: ${cartItem.product.id}');
    print('🛒 [DEBUG] PriceTag ID: ${cartItem.priceTag.id}');
    
    final body = jsonEncode(cartItem.toBodyJson());
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      print('❌ [DEBUG] Cart add failed with status: ${response.statusCode}');
