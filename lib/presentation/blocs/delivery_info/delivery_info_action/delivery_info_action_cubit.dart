...
class DeliveryInfoActionCubit extends Cubit<DeliveryInfoActionState> {
  ...
  void addDeliveryInfo(DeliveryInfoModel params) async {
    try {
      print('📍 [DEBUG] Adding delivery info...');
      print('📍 [DEBUG] Customer name: ${params.customerName}');
      print('📍 [DEBUG] Customer email: ${params.customerEmail}');
      print('📍 [DEBUG] Customer phone: ${params.customerPhone}');
      print('📍 [DEBUG] Shipping address: ${params.shippingAddress}');
      
      emit(DeliveryInfoActionLoading());
      final result = await _deliveryInfoAddUsecase(params);
      result.fold(
        (failure) {
          print('❌ [DEBUG] Add delivery info failed: $failure');
          emit(DeliveryInfoActionFail());
        },
        (deliveryInfo) {
          print('✅ [DEBUG] Delivery info added successfully!');
          emit(DeliveryInfoAddActionSuccess(deliveryInfo));
        },
      );
    } catch (e, stackTrace) {
      print('❌ [DEBUG] Exception in addDeliveryInfo: $e');
